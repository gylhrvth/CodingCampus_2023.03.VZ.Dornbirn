...
public class UseListPerson {
    ...
    enum SortBy {
        NAME(Comparator.comparing(Person::getName)), AGE(Comparator.comparing(Person::getAge)), WEIGHT(Comparator.comparing(Person::getWeight)), HEIGHT(Comparator.comparing(Person::getHeight));
        private Comparator<Person> comp;
        SortBy(Comparator<Person> comp) {
            this.comp = comp;
        }
    }
