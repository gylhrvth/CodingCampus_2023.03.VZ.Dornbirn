...
public class Main {
    ...
    public static void printRecrusive(String space, File f, boolean last, int depth) {
        ...
        if (!last) {
            System.out.println(space + "├──" + "(" + depth + ")" + (f.isFile() ? Colors.COLORS[1] : Colors.COLORS[4])
...
//            for (File fi : files) {
            for (int i = 0; i < files.length; i++) {
                ...
                if (i + 1 < files.length) {
                    printRecrusive(newPrefix, files[i], false, depth);
                } else {
                    printRecrusive(newPrefix, files[i], true, depth);
                }
            }
        }
